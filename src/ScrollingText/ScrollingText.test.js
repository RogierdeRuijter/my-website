...
describe('ScrollingText', () => {
  ...
  const testFishRaceFlow = () => {
    const firstSentence_1 = screen.getByText('Gemaakt in een avond vol met drank door Daniel en mij');
    expect(firstSentence_1).toBeInTheDocument();
    expect(firstSentence_1).toHaveClass('active-text');

    const secondSentence_1 = screen.getByText('Later verfijnd door de mensen die hiervoor genoemd zijn en Joost');
    expect(secondSentence_1).toBeInTheDocument();
    expect(secondSentence_1).toHaveClass('next-text');

    act(() => jest.advanceTimersByTime(5500));

    const secondSentence_2 = screen.getByText('Later verfijnd door de mensen die hiervoor genoemd zijn en Joost');
    expect(secondSentence_2).toBeInTheDocument();
    expect(secondSentence_2).toHaveClass('active-text');

    const thirdSentence_2 = screen.getByText('Bedankt jongens voor de eindeloze discussies over de essentie van de Fish race');
    ...
    const thirdSentence_3 = screen.getByText('Bedankt jongens voor de eindeloze discussies over de essentie van de Fish race');
    ...
    const thirdSentence_4 = screen.getByText('Bedankt jongens voor de eindeloze discussies over de essentie van de Fish race');
  ...
  it.only('should show the fish race description in order', () => {
    ...
    const firstSentence_0 = screen.getByText('Gemaakt in een avond vol met drank door Daniel en mij');
